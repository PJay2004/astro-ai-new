...
function getCountryFromTimezone(
  ...
  if (tzToCountry[timezone]) {
    return { name: tzToCountry[timezone] };
  }

  const parts = timezone.split("/");
  const last = parts.pop() || timezone;
  const city = last.replace(/_/g, " ");
  const region = parts.length ? parts.join("/") : undefined;
        ...
        items: [
          ...
          },
          {
            ...
            value: contactSession.metadata.currentUrl ? (
              <a
                href={contactSession.metadata.currentUrl}
                target="_blank"
    ...
    <div className="flex h-full w-full flex-col bg-background text-foreground">
      ...
      <div>
        {contactSession.metadata && (
          <Accordion
            className="w-full rotate-none border-y"
            collapsible
            type="single"
          >
            {accordionSections.map((section) => (
              ...
              >
                <AccordionTrigger
                  className="flex w-full flex-1 items-start justify-between gap-4 rotate-none bg-accent px-5 py-4 text-left font-medium text-sm 
                                              outline-none transition-all hover:no-underline disabled:pointer-events-none disabled:opacity-50"
                >
