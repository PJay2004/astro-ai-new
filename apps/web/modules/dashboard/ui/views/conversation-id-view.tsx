...
export const ConversationIdView = ({
  ...
  const [isEnchancing, setIsEnchancing] = useState(false);
  const echanceResponce = useAction(api.private.messages.echanceResponce);
  const handleEnchanceResponce = async () => {
    setIsEnchancing(true);
    const currentValue = form.getValues("message");

    try {
      const response = await echanceResponce({ prompt: currentValue });
        ...
        <Form {...form}>
          ...
          >
            ...
            <AIInputToolbar>
              <AIInputTools>
                <AIInputButton
                  onClick={handleEnchanceResponce}
                  disabled={
                    conversation?.status === "resolved" ||
                    isEnchancing ||
                ...
                >
                  <Wand2Icon />
                  {isEnchancing ? "Enhancing..." : "Enhance"}
                </AIInputButton>
              </AIInputTools>
              <AIInputSubmit
                disabled={
